...
//TODO fill table out with measured values.
	private static final double [][] kInchesToSpeed = new double[][] {
		{0, 0},
		{1, 1000},
		{2, 2000},
		{3, 3000},
...
https://github.com/CrossTheRoadElec/FRC-Examples/blob/master/JAVA_VelocityClosedLoop/src/org/usfirst/frc/team469/robot/Robot.java */
	...
	private static double inchesToRpm(double inches) {
		...
		while (kInchesToSpeed.length + 1 > index) {
			...
			} else if (kInchesToSpeed[index+1][0] == inches ) {
				return kInchesToSpeed[index+1][1];
			} else if ((inches >  kInchesToSpeed[index][0]) && (kInchesToSpeed[index][0] > inches)) {
				break;
			}
			++index;
		}
		...
		if (index+1 >= kInchesToSpeed.length) {
			System.err.println("Ran off the end of the Inches to speed Table");
			return 0;
		}
		...
		// b = y + mx
		// y = mx + b
		double m = (kInchesToSpeed[index+1][1] - kInchesToSpeed[index][1]) / (kInchesToSpeed[index+1][0] - kInchesToSpeed[index][0])  ;
		double b =  kInchesToSpeed[index][1] + (m * kInchesToSpeed[index][0]);
		double y = (m * kInchesToSpeed[index][0]) + b;
		return y;
	}
