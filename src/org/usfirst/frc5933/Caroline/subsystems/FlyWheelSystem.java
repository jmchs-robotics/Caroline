...
public class FlyWheelSystem extends Subsystem {
	...
	private static final double[][] kInchesToSpeed = new double[][] { { 0, 0 }, { 1, 1000 }, { 2, 2000 }, 
	...
	public final static double kSpeedIncrement = 50;
	public final static double kSpeedMin = 1800;
	...
	private void goSpeed() {
		...
		if(using_vision){
			...
			double inches = Robot.visionBoiler_.get_distance();
		...
		SmartDashboard.putNumber("Vision-based distance: ", Robot.visionBoiler_.get_distance());
	...
	/*
	 * This method returns a 1 dimensional array that contains
	 * appropriate values for the flywheel to set itself to: 
	 * rpm, f, p, i, and d values are all returned for a specialized method
	...
	private static double[] inchesToRpmPIDF(double inches) {
		...
		while (kLookupFlywheelSettings.length - 1 > index) {
			...
			} else if ((inches > kLookupFlywheelSettings[index][0]) && (kLookupFlywheelSettings[index + 1][0] > inches)) {
				break;
			}
			++index;
		}
		...
		//the trick is to do this with each element in the subarray, and to build
