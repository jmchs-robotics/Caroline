...
public class HopperSystem extends Subsystem {
	...
	private static final boolean useAccel = true;

	private static final double kImpactThreshold = 1.0;	//TEST THIS #
	...
	public void teleopPeriodic() {
		if (running_) {
			if(useAccel){									//driver preference whether they want this assistance or not.
				if(accelerometerOver(kImpactThreshold)){	//if combined acceleration vector tops threshold,
					agitiatorMotor.set(0);					//stop shooting (agitation) immediately,
					running_ = !running_;					//toggle running,
					return;									//and get out of here, you don't need to finish this method.
				}
			}
	...
	public void autonomousPeriodic() {
		if (running_) {
			if(useAccel){		//driver preference whether they want this assistance or not.
				if(accelerometerOver(kImpactThreshold)){		//if either left or right acceleration tops 0.5,
					agitiatorMotor.set(0);		//stop shooting (agitation) immediately
					running_ = !running_;		//toggle running
					return;						//get out of here, you don't need to finish this method.
				}
			}
			agitiatorMotor.set(agitatorSpeed_);
		} else {
	...
	private boolean accelerometerOver(double threshold){
		...
		double totalMagnitude = Math.sqrt(x*x + y*y + z*z);		//combine the three into one vector. If you EVER get over 'gs', cut the hopper
